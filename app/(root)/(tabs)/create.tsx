...
import {
  ...
  TouchableOpacity,
  View,
  useColorScheme, // Ajout pour la gestion des icônes et placeholders natifs
} from "react-native";
...
// Adaptation sémantique globale des classes d'entrée
const inputClass =
  "bg-card border border-border rounded-2xl px-4 py-3 text-card-foreground";
const labelClass = "text-sm font-semibold text-card-foreground mb-1.5";
...
export default function CreatePropertyScreen() {
  ...
  // Variables dynamiques pour les composants non-CSS
  ...
  const Counter = ({
    ...
    <View className="flex-1">
      ...
      <View className="flex-row items-center bg-card border border-border rounded-2xl overflow-hidden">
        ...
        <Text className="flex-1 text-center text-card-foreground font-bold text-base">
          {value}
        </Text>
        <TouchableOpacity
  ...
  const Toggle = ({
    ...
    <TouchableOpacity
      ...
      className={`flex-row items-center justify-between p-4 rounded-2xl border ${
        value ? "bg-blue-500/10 border-border" : "bg-card border-border"
      }`}
    >
      <View className="flex-1 mr-3">
        {/* Remplacement de text-foreground par text-card-foreground ici */}
        <Text
          className={`font-semibold ${
            value ? "text-card-foreground" : "text-card-foreground"
          }`}
        >
          {label}
        </Text>
        {description && (
          <Text className="text-xs text-card-foreground mt-0.5">
            {description}
          </Text>
        )}
      </View>
      <View
        className={`w-8 h-8 rounded-full border-2 items-center justify-center ${
          value ? "bg-blue-500 border-border" : "border-border"
        }`}
      >
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      >
        ...
        {/* Header */}
        <View className="flex-row -mt-4 items-center px-5 pt-4 pb-3">
          <Text className="text-2xl font-bold text-card-foreground flex-1">
            Add Property
          </Text>
        </View>
        ...
        >
            ...
            <View className="flex-row flex-wrap gap-3">
              {form.localImages.map((uri, index) => (
                <View key={index} className="relative">
                  ...
                  />
                  {index === 0 && (
                    <View className="absolute top-1 left-1 bg-accent px-1.5 py-0.5 rounded-full">
                      <Text className="text-card-foreground text-[9px] font-bold">
                        COVER
                      </Text>
                    </View>
                  )}
              ...
              {form.localImages.length < 6 && (
                <TouchableOpacity
                  ...
                  className="w-24 h-24 rounded-2xl bg-card border-2 border-dashed border-border items-center justify-center"
                >
                  {uploadingImages ? (
                    ...
                    <>
                      ...
                      />
                      <Text className="text-card-foreground text-xs mt-1">
                        Add
                      </Text>
                    </>
                  )}
          ...
          <View className={sectionClass}>
            ...
            />
            <Text className="text-xs text-card-foreground mt-1.5 ml-1">
                ...
                >
                  <Text
                    className={`text-sm font-semibold capitalize ${
                      form.type === t ? "text-card" : "text-card-foreground"
                    }`}
                  >
                    {t}
                  </Text>
          ...
          <TouchableOpacity
            ...
            className="bg-accent rounded-2xl py-4 items-center border border-border"
            style={{
